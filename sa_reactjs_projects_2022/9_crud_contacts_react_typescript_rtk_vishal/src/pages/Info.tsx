...
const Info = () => {
  const { id } = useParams();
  const { data, error } = useContactQuery(id!); // Bring our query and pass id here
  // a postfix ! removes null and undefined from the type of an expression.
  ...
  // TypeScript that you're certain that value is not null or undefined.
  useEffect(() => {
    if (error) {
